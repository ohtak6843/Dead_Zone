...
void Stage03::LoadResources()
{
	// 맵 로드
	GET_SINGLE(Resources)->LoadFBX(L"..\\Resources\\FBX\\Airport.fbx");
	GET_SINGLE(Resources)->LoadFBX(L"..\\Resources\\FBX\\Airport_Wall.fbx");
...
#pragma region Local Player
	{
		...
		localPlayer->GetTransform()->SetLocalPosition(Vec3(1000.f, 140.f, 1000.f));
...
#pragma region Map
	{
		shared_ptr<MeshData> meshData = GET_SINGLE(Resources)->LoadFBX(L"..\\Resources\\FBX\\Airport.fbx");
	...
	{
		shared_ptr<MeshData> meshData = GET_SINGLE(Resources)->LoadFBX(L"..\\Resources\\FBX\\Airport_Wall.fbx");
		vector<shared_ptr<GameObject>> gameObjects = meshData->Instantiate(ColliderType::OBB);
