...
void Material::PushGraphicsData()
{
	...
	for (size_t i = 0; i < _textures.size(); i++)
	{
		...
		gameFramework->GetGraphicsDescHeap()->SetSRV(_textures[i]->GetSRVHandle(), reg);
	}
...
void Material::PushComputeData()
{
	...
	for (size_t i = 0; i < _textures.size(); i++)
	{
		...
		SRV_REGISTER reg = SRV_REGISTER(static_cast<int8>(SRV_REGISTER::t0) + i);
		gameFramework->GetComputeDescHeap()->SetSRV(_textures[i]->GetSRVHandle(), reg);
	}
...
void Material::Dispatch(uint32 x, uint32 y, uint32 z)
{
	...
	gameFramework->GetComputeDescHeap()->CommitTable();

	COMPUTE_CMD_LIST->Dispatch(x, y, z);

	gameFramework->GetComputeCmdQueue()->FlushComputeCommandQueue();
}
